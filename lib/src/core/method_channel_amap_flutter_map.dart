...
class MethodChannelAMapFlutterMap implements AMapFlutterPlatform {
  ...
  Stream<LocationChangedEvent> onLocationChanged({required int mapId}) {
    return _events(mapId)
        .where((event) => event is LocationChangedEvent)
        .cast<LocationChangedEvent>();
  }

  //Camera 移动回调
  Stream<CameraPositionMoveEvent> onCameraMove({required int mapId}) {
    return _events(mapId)
        .where((event) => event is CameraPositionMoveEvent)
        .cast<CameraPositionMoveEvent>();
  }

  ///Camera 移动结束回调
  Stream<CameraPositionMoveEndEvent> onCameraMoveEnd({required int mapId}) {
    return _events(mapId)
        .where((event) => event is CameraPositionMoveEndEvent)
        .cast<CameraPositionMoveEndEvent>();
  }
  ...
  Stream<MapPoiTouchEvent> onPoiTouched({required int mapId}) {
    return _events(mapId)
        .where((event) => event is MapPoiTouchEvent)
        .cast<MapPoiTouchEvent>();
  }
